...
export function BlackHoleHeroSection({
  ...
  // The render loop reads its settings from this ref, so a prop change retunes
  // the picture without tearing down the WebGL context and rebuilding every
  // shader. Written in an effect rather than during render: a ref mutated mid
  // render is a rule violation, and the loop only reads it on the next frame
  // anyway, so there is nothing to gain by writing it earlier.
  useEffect(() => {
    props.current = {
      distance, elevation, azimuth, orbitSpeed, roll, fov, diskInner, diskOuter,
      diskThickness, diskDensity, brightness, spinSpeed, grain, doppler, hotColor,
